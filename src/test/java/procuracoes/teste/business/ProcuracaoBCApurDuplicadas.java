...
public class ProcuracaoBCApurDuplicadas {
	...
	public void init() {
		...
		logger = LogManager.getLogger(ProcuracaoBCApurDuplicadas.class.getName());
		...
		try {
			...
			for (String cdControl : codControle) {
				...
				for (Procuracao procuracao : procs) {
					System.out.println(procuracao.getNumeroControle() + "," + procuracao.getDataConferencia() + "," + procuracao.getCpfConferencia() + "," + procuracao.getCdUaAtendimento()  );
...
//					logger.info(json.toJson(log));
					logger.info(log.getIsn() +","+ log.getCodigoControtrole() + "," + log.getNomeOperacao());
				}
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
...
//	 @Test
	public void obterProcuracaoIsn() {
		ProcuracaoBC bc = ProcuracaoBC.getInstance();
		Gson json = new Gson();
		ArrayList<String> isnProcuracao = new ArrayList<>();
		isnProcuracao.add("16644892");		
		try {
			bc.setAmbiente(envNumber);
